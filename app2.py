...
def fetch_references(condition: str) -> list:
    trusted_domains = [
        "mayoclinic", "healthline", "webmd", "medlineplus",
        "nhs.uk", "nih.gov", "clevelandclinic", "hopkinsmedicine",
        "medicalnewstoday", "who.int"
    ]
    queries = [
...
def is_emergency(text: str) -> bool:
    flags = [
        ...
        "seizure", "severe bleeding", "suicide", "overdose",
        "anaphylaxis", "not breathing", "coughing blood", "vomiting blood",
...
class MediPDF(FPDF):
    ...
    def footer(self):
        ...
        self.cell(0, 4,
            "If Confused, also consult with a Human Doctor", align="C")
...
def make_pdf(title: str, content: str, profile: dict,
             report_type: str = "Report", references: list = None) -> bytes:
    ...
    pdf.set_x(15)
    pdf.cell(0, 5, f"Generated: {datetime.datetime.now().strftime('%d %b %Y  |  %I:%M %p')}",
             ln=True, align="R")
    pdf.ln(2)
    ...
    pdf.set_x(18)
    pdf.cell(0, 4.5, f"Name: {name}    Age: {age}    Gender: {gen}    Location: {loc}", ln=True)
    pdf.set_x(18)
...
with st.sidebar:
    ...
    p["name"]       = st.text_input("Full Name", value=p["name"], placeholder="e.g. Rohan Sharma")
    p["age"]        = st.text_input("Age", value=p["age"], placeholder="e.g. 28")
    p["gender"]     = st.selectbox("Gender", ["Male", "Female", "Other"],
                                    index=["Male","Female","Other"].index(p.get("gender","Male")))
    p["location"]   = st.text_input("Location", value=p["location"],placeholder="City, State")
    p["conditions"] = st.text_input("Known Conditions", value=p.get("conditions",""), placeholder="e.g. Diabetes")
    p["allergies"]  = st.text_input("Allergies", value=p.get("allergies",""), placeholder="e.g. Penicillin")
...
if p["location"]: subtitle_parts.append(f"📍 {p['location']}")
if p["age"]:      subtitle_parts.append(f"🎂 Age {p['age']}")
st.caption("  •  ".join(subtitle_parts))
st.info("⚡ Available 24/7 — Powered by Gemini 2.5 Flash")
st.divider()


tab1, tab2, tab3 = st.tabs(["🩺 Virtual Clinic", "📋 Report Analyzer", "💬 Medical Chatbot"])
...
with tab1:
    ...
    with left_col:
        ...
        with btn_col1:
            go = st.button("🔍 Start Analysis", use_container_width=True, type="primary")
        with btn_col2:
        ...
        if go and symptoms_input and not is_emergency(symptoms_input):
            ...
            st.session_state.symptom_chat     = [{"role": "user", "content": symptoms_input}]
...
Do NOT diagnose yet.""")
        ...
        if st.session_state.symptom_chat:
            ...
            for msg in st.session_state.symptom_chat:
                if msg["role"] == "user":
                    st.markdown(f'<div class="bubble-user"><strong>You:</strong><br>{msg["content"]}</div>',
                                unsafe_allow_html=True)
                else:
                    st.markdown(f'<div class="bubble-ai"><strong>🤖 MediHelp AI:</strong><br>{msg["content"]}</div>',
                                unsafe_allow_html=True)

        # Stage 2 — collect answers, generate verdict
        if st.session_state.symptom_stage == "followup":
            answers = st.text_area("Your answers to the follow-up questions:",
                                   placeholder="1. Started yesterday...\n2. Severity is 6/10...",
                                   height=120)
            if st.button("✅ Get Full Assessment", use_container_width=True, type="primary"):
                if answers.strip():
                    st.session_state.symptom_chat.append({"role": "user", "content": answers})
                    with st.spinner("Generating your personalized health assessment..."):
...
Be warm, clear, evidence-based.""")
                    st.session_state.symptom_chat.append({"role": "ai", "content": verdict})
                    st.session_state.verdict_text  = verdict
                    st.session_state.symptom_stage = "verdict"
                    st.rerun()

        # Stage 3 — PDF download
        if st.session_state.symptom_stage == "verdict" and st.session_state.verdict_text:
            st.divider()
            pdf_bytes = make_pdf("Virtual Clinic Assessment",
                                 st.session_state.verdict_text, p, "Virtual Clinic Report")
            st.download_button("📄 Download Assessment as PDF",
                               data=pdf_bytes, file_name="medihelp_clinic.pdf",
...
with tab2:
    ...
    st.caption("Upload a scan or paste lab results — AI translates jargon into plain English. References + PDF included.")
    ...
    if "Image" in report_mode:
        ...
        with prev_col:
            if uploaded:
                ...
                st.image(Image.open(io.BytesIO(st.session_state["_img_bytes"])),
                         caption=uploaded.name, use_container_width=True)

        if uploaded:
            if st.button("🔬 Analyze Image", use_container_width=True, type="primary"):
                with st.spinner("🧠 AI is reading your scan — usually 15-30 seconds..."):
                    result = gemini_image(f"""You are a consultant radiologist.
...
## Important Note (confirm AI must be reviewed by licensed radiologist)""",
        ...
        if st.button("🔬 Analyze Report", use_container_width=True,
                     key="analyze_text_btn", type="primary"):
...
## Plain English Summary (3 sentences max)""")
    ...
    if st.session_state.report_analysis:
        ...
        with dl1:
            pdf_bytes = make_pdf("Medical Report Analysis",
                                 st.session_state.report_analysis,
                                 p, "Report Analysis",
                                 references=refs or None)
            st.download_button("📥 Download PDF (with references)",
                               data=pdf_bytes, file_name="medihelp_report.pdf",
                               mime="application/pdf", use_container_width=True)
        with dl2:
            ...
            st.download_button("📝 Download as Text",
                               data=txt, file_name="medihelp_report.txt",
...
with tab3:
    ...
    st.caption("Ask anything about health, anatomy, medications, lab values, nutrition — science-backed answers.")
...
End with: 'For personal decisions, consult your doctor.'""")
    ...
    if st.session_state.chat_history:
        ...
        st.info("💬 Ask me anything about health science, medications, lab values, symptoms, nutrition, and more.")
...
- Max 300 words.""")
    ...
    if st.session_state.chat_history:
        ...
        with ac2:
            ...
            st.download_button("📄 Export as PDF", data=pdf_bytes,
